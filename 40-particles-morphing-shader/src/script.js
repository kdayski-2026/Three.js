...
let particles
const promises = [
    new Promise((resolve) => gltfLoader.load('./models.glb', (gltf) => resolve(gltf))),
    new Promise((resolve) => gltfLoader.load('./stul.glb', (gltf) => resolve(gltf)))
]
Promise.all(promises).then((files) => {
    const models = files.flatMap(model => model.scene.children)

    models[3].scale.setScalar(0.05)
    models[3].position.x = -0.25
    models[3].position.y = -1.5
    models[3].updateWorldMatrix(true, false)
    const geo = models[3].geometry.clone()
    geo.applyMatrix4(models[3].matrixWorld)
    models[3].geometry = geo

    particles = {}
    particles.index = 0

    // Positions
    const positions = models.map((child) => child.geometry.attributes.position)
    ...
    particles.material = new THREE.ShaderMaterial({
        ...
        uniforms:
        {
            ...
            uTime: new THREE.Uniform(elapsed)
        },
    ...
    // Tweaks
    gui.add(particles.material.uniforms.uProgress, 'value', 0, 1, 0.001).name('uProgress').listen()
