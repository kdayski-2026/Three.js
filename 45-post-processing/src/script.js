...
const switchPass = (target) => {
    if (!target) {
        effectComposer.removePass(target)
        previousPass.enabled = false
        return
    }
    if (previousPass) {
        effectComposer.removePass(target)
    ...
    target.enabled = true
    effectComposer.addPass(target)
    previousPass = target
}
gui.add(debugObject, 'pass', debugObject.passes).onChange((target) => switchPass(target))
